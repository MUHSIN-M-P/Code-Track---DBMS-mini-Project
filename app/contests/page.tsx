...
export default function ContestsPage() {
  ...
  const upcoming = contests.filter(
    (c) => getStatus(c).label === "Upcoming"
  );
  const active = contests.filter(
    (c) => getStatus(c).label === "Active"
  );
  const ended = contests.filter(
    (c) => getStatus(c).label === "Ended"
  );
            ...
            <div key={c.id} className="contest-card">
              <div className="contest-header">
                <div>
                  ...
                  >
                    <h3 className="contest-title table-link">{c.title}</h3>
                  </Link>
                  <p
                    style={{
                      fontSize: 14,
