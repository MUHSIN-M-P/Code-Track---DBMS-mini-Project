...
interface User { id: string; name: string; email: string; role: string; joinedAt: string; totalSolved: number; totalSubmissions: number; acceptanceRate: number; }
interface Contest { id: string; title: string; startTime: string; duration: number; _count: { registrations: number; problems: number }; }
...
export default function AdminPage() {
  ...
  const loadData = () => {
    fetch("/api/leaderboard").then((r) => r.json()).then(setUsers);
  ...
  const createContest = async () => {
    ...
    setCTitle(""); setCDesc(""); setCStart(""); setCDuration("90");
    loadData();
  };
        ...
        {tab === "contests" && (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              ...
              <Button color="primary" onClick={() => setShowContestModal(true)} className="flex items-center gap-2 shadow-md">
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-200">
                      ...
                      <th className="px-6 py-4 text-sm font-semibold text-gray-700">Problems</th>
                      <th className="px-6 py-4 text-sm font-semibold text-gray-700">Entries</th>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {contests.map((c) => (
                      <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-sm text-gray-600">{new Date(c.startTime).toLocaleString()}</td>
                        <td className="px-6 py-4 text-gray-700">{c.duration} min</td>
                        <td className="px-6 py-4 text-gray-700">{c._count?.problems || 0}</td>
                        <td className="px-6 py-4 text-gray-700">{c._count?.registrations || 0}</td>
                        <td className="px-6 py-4 text-right">
                          <button onClick={() => deleteContest(c.id)} className="p-2 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors">
                            <Trash2 size={16} />
                          </button>
                        </td>
                      </tr>
                    ...
                    {contests.length === 0 && (
                      <tr><td colSpan={6} className="px-6 py-8 text-center text-gray-500">No contests defined</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
              ...
              <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 overflow-y-auto backdrop-blur-sm">
                <Card className="w-full max-w-lg border border-gray-200 bg-white my-8 shadow-2xl">
                  <CardHeader className="flex flex-row justify-between items-center border-b border-gray-200 pb-4">
                    ...
                    <button onClick={() => setShowContestModal(false)} className="text-gray-400 hover:text-gray-600 p-1 transition-colors">
                      <X size={20} />
                    </button>
                  </CardHeader>
                  <CardContent className="pt-6 space-y-4">
                    <div className="space-y-2">
                      ...
                      <Input value={cTitle} onChange={(e) => setCTitle(e.target.value)} placeholder="Weekly Challenge #2" className="w-full border-gray-300 bg-white text-gray-900" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-sm font-medium text-gray-700">Description</label>
                      <textarea
                        className="w-full border border-gray-300 bg-white text-gray-900 rounded-lg p-3 min-h-[80px] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        value={cDesc}
                    ...
                    <div className="space-y-2">
                      ...
                      <Input type="datetime-local" value={cStart} onChange={(e) => setCStart(e.target.value)} className="w-full border-gray-300 bg-white text-gray-900" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-sm font-medium text-gray-700">Duration (min)</label>
                      <Input type="number" value={cDuration} onChange={(e) => setCDuration(e.target.value)} className="w-full border-gray-300 bg-white text-gray-900" />
                    </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-200">
                      ...
                      <th className="px-6 py-4 text-sm font-semibold text-gray-700">Total Solved</th>
                      <th className="px-6 py-4 text-sm font-semibold text-gray-700">Total Submissions</th>
                      <th className="px-6 py-4 text-sm font-semibold text-gray-700">Acceptance Rate</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-200">
                    {users.map((u) => (
