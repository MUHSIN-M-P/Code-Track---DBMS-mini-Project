...
export default function ContestDetailPage() {
  ...
  const isRegistered = contest.registrations.some((r) => r.user.id === userId);
  ...
  const handleRegister = async () => {
    ...
    if (res.ok) {
      const updated = await fetch(`/api/contests/${id}`).then((r) => r.json());
      setContest(updated);
    } else {
                ...
                <tbody>
                  {contest.problems.map((cp) => (
                    <tr key={cp.problem.id}>
                      ...
                      <td>
                        <span className={`badge ${diffBadge[cp.problem.difficulty]}`}>
                          {cp.problem.difficulty}
                        </span>
                      </td>
                      <td>
                        <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                          {cp.problem.topics.map((t) => (
                            <span key={t.topic.name} className="badge badge-topic">
                              {t.topic.name}
                            </span>
                          ))}
                        </div>
